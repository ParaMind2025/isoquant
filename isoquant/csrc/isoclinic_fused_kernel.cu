    ...
    int n_levels)
{
    ...
    isoclinic_full_fused_kernel<T, LEVELS><<<batch_size, threads>>>(
        input.data_ptr<T>(),
        q_left.data_ptr<float>(),
        q_right.data_ptr<float>(),
    ...
    int n_levels)
{
    switch (n_levels) {
        case 4:
            return isoclinic_full_fused_impl<T, 4>(input, q_left, q_right, centroids, n_levels);
        case 8:
            return isoclinic_full_fused_impl<T, 8>(input, q_left, q_right, centroids, n_levels);
        case 16:
            return isoclinic_full_fused_impl<T, 16>(input, q_left, q_right, centroids, n_levels);
        default:
            return isoclinic_full_fused_impl<T, 0>(input, q_left, q_right, centroids, n_levels);
    }
}
    ...
    int n_levels)
{
    switch (n_levels) {
        case 4:
            return isoclinic_fast_fused_impl<T, 4>(input, q_left, centroids, n_levels);
        case 8:
            return isoclinic_fast_fused_impl<T, 8>(input, q_left, centroids, n_levels);
        case 16:
            return isoclinic_fast_fused_impl<T, 16>(input, q_left, centroids, n_levels);
        default:
            return isoclinic_fast_fused_impl<T, 0>(input, q_left, centroids, n_levels);
    }
}
